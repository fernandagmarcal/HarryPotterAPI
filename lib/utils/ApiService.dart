...
class ApiResponse<T> {
  final T? data;
  final int
  statusCode; //se o code vinher com error não mexe com data por isso que ele pode ser null

  ApiResponse({required this.data, required this.statusCode});
}

class ApiService {
  //Static não precisa do metodo para instanciar
  static Future<ApiResponse<T>> request<T>({
    //Future app não vai esperar o Future terminar para seguir. Pode colocar um item de load
    required String url,
